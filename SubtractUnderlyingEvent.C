...
void SubtractUnderlyingEvent(TString inputPath, TString inputPrefix, TString inputMiddle, TString inputEnd, TString uePath, TString uePrefix, TString ueMiddle, TString ueEnd, TString outputPath, Int_t sysErrorAddition = kLinear, TString fileToRebin = "", Bool_t compare = kFALSE) { 
...
//   gROOT->LoadMacro("~/Documents/macros/AddUpSystematicErrors.C+");
  ...
  const Int_t startMode = 0;
  const Int_t endMode = 2;
  
  const Int_t startJetPtBin = 0;
  const Int_t endJetPtBin = 4;
  
//   TString ending = ".root";
// 	
//   if (inputEnd.EndsWith(".root"))
//     inputEnd.Remove(TString::kTrailing,ending.Data());
//   if (ueEnd.EndsWith(".root"))
//     ueEnd.Remove(TString::kTrailing,ending.Data());
  
  TString outputPrefix = inputPrefix;
  TString outputMiddle = inputMiddle;
  TString outputEnd = inputEnd;
  outputEnd.Append("_UEsubtracted");
  
  if (inputPrefix.Contains("outputSystematics", TString::kIgnoreCase) && sysErrorAddition == kQuadratic)
    	outputEnd.Append("_QuadraticSysAddition");
  if (inputPrefix.Contains("outputSystematics", TString::kIgnoreCase) && sysErrorAddition == kLinear)
      outputEnd.Append("_LinearSysAddition");
  
  TString inputName, ueName, outputName;
  ...
  Bool_t efficiencyCorrected = inputPrefix.Contains("EfficiencyCorrection");
  ...
  TH1 *yieldHistogramsOriginal[nOfYieldHistograms];
  TH1 *yieldHistogramsOriginal_sysErrors[nOfYieldHistograms];
  
  TH1 *yieldHistogramsOriginalMC[nOfYieldHistograms];
  
  TH1 *yieldHistogramsUE[nOfYieldHistograms];
  TH1 *yieldHistogramsUE_sysErrors[nOfYieldHistograms];
  
  TH1 *yieldHistogramsUEMC[nOfYieldHistograms];
  
  TH1 *yieldHistogramsUEsubtracted[nOfYieldHistograms];
  
  TH1 *yieldHistogramsUEsubtractedMC[nOfYieldHistograms];
  TH1 *ratioToPiHistogramsUEsubtractedMC[nOfYieldHistograms];
  
  TH1 *totalYieldHistograms[nOfTotalYieldHistograms];
	
  TH1* yieldHistogramFractions[nOfYieldHistograms];
  TH1* ratioToPiHistograms[nOfYieldHistograms];
  
  //Systematic Errors 
  TGraphAsymmErrors* systematicErrorYieldsOriginal[nOfYieldHistograms];
  TGraphAsymmErrors* systematicErrorYieldsUE[nOfYieldHistograms];
  TGraphAsymmErrors* systematicErrorYieldsUEsubtracted[nOfYieldHistograms];
  TGraphAsymmErrors* systematicErrorUEsubtracted[nOfYieldHistograms];
  TGraphAsymmErrors* systematicErrorToPiRatioUEsubtracted[nOfYieldHistograms];
  ...
  Bool_t useSysError[nOfYieldHistograms];
	
  //Event Histograms
  
  TH1D *eventHistogramsOriginal[nOfEventHistograms];
  
  TH2D *jetHistogramsOriginal[nOfJetsHistograms];
	
  TH1D *eventHistogramsUEsubtracted[nOfEventHistograms];
  
  TH2D *jetHistogramsUEsubtracted[nOfJetsHistograms];
  
  TH1::SetDefaultSumw2();
  
  
  for (Int_t iMode = startMode; iMode < endMode; iMode++) {
    for (Int_t iJetPt = startJetPtBin; iJetPt < endJetPtBin; iJetPt++) {
      for (Int_t k=0;k<nOfYieldHistograms;++k) {
        if (sysErrorAddition == kNosysErrorCalculation)
          useSysError[k] = kFALSE;
        else
          useSysError[k] = kTRUE;
      }
   
      //Set File names
      inputName = TString::Format("%s/%s%s_%s_jetPt%i.0_%i.0%s.root",inputPath.Data(), inputPrefix.Data(), mode[iMode].Data(), inputMiddle.Data(),jetPt[iJetPt], jetPt[iJetPt + 1], inputEnd.Data());
      ueName = TString::Format("%s/%s%s_%s_jetPt%i.0_%i.0%s.root",uePath.Data(), uePrefix.Data(), mode[iMode].Data(), ueMiddle.Data(),jetPt[iJetPt], jetPt[iJetPt + 1], ueEnd.Data());
      outputName = TString::Format("%s/%s%s_%s_jetPt%i.0_%i.0%s.root",outputPath.Data(), outputPrefix.Data(), mode[iMode].Data(), outputMiddle.Data(),jetPt[iJetPt], jetPt[iJetPt + 1], outputEnd.Data());
...
//             hRatioToPiCorrectedSysError[k]->Draw("E2 same");
      ...
      if (efficiencyCorrected && sysErrorAddition != kNosysErrorCalculation) {
        cout << "Construct sys Errors, if wished " << endl;
      ...
      if (useSysError[nOfPionHistogram] && (systematicErrorYieldsOriginal[nOfPionHistogram]->GetN() == systematicErrorYieldsUE[nOfPionHistogram]->GetN())) {
      ...
      for (Int_t k=0;k<nOfYieldHistograms;++k) {
        if (k == nOfPionHistogram || !useSysError[k] || (systematicErrorYieldsOriginal[k]->GetN() != systematicErrorYieldsUE[k]->GetN()))
          continue;
          ...
          if (rebinHistogram) {
            ...
            if (compare) {
              ...
              if (compareHistogramSignal) {
                ratioSignal = new TH1("h1ratioSignal", "Ratio of Signal results",numberOfNewBins,xbinsRebinned);
              ...
              if (compareHistogramUE) {
                ratioUE = new TH1("h1ratioUE", "Ratio of UE results",numberOfNewBins,xbinsRebinned);
