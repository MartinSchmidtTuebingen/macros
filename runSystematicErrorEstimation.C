    ...
    for (Int_t iJetPt = 0; iJetPt <= maxJetPtBins-1; iJetPt++)  {
      ...
      for (Int_t mode = 0; mode <= maxMode-1; mode++) {  
        ...
        if (!sysetapath.IsNull())
        {
          ...
          histTitles[1] = "#eta correction - 0.5%, p > 0.45 GeV/c; - %3, p < 0.45 GeV/c";
          histTitles[2] = "#eta correction + 0.5%, p > 0.45 GeV/c; + %3, p < 0.45 GeV/c";
        ...
        if (!syssplinespath.IsNull())
        {
          ...
          histTitles[1] = "Splines - 0.6% for #beta#gamma > 50; -0.3% for #beta#gamma < 50%";
          histTitles[2] = "Splines + 1.3% for #beta#gamma > 50; +0.3% for #beta#gamma < 50%";
